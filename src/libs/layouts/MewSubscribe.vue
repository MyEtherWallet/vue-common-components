<template>
    <div>
        ...
        <transition enter-active-class="transform ease-out duration-500 transition opacity-0 delay-100"
            enter-to-class="opacity-100" leave-active-class="opacity-0" leave-to-class="opacity-0" appear>
            <div v-if="model"
                class="cursor-pointer fixed inset-0 bg-black/40 z-[99] h-full w-screen overflow-none overscroll-none"
                @click="setIsOpen(false, 0, 'click-outside-popup')" aria-hidden />
        </transition>
        <!-- Dialog Container -->
        <div v-if="model" class="cursor-pointer fixed inset-0 h-screen flex items-center justify-center p-9 z-[100]"
            @click="setIsOpen(false, 0, 'click-outside-popup')">
            <transition enter-active-class="transform ease-out duration-500 transition opacity-0 delay-100"
                enter-to-class="opacity-100" leave-active-class="opacity-0" leave-to-class="opacity-0" appear>
                <div v-if="model"
                    class="cursor-default fixed  h-[95%] sm:h-[512px] w-[95%] md:w-[800px] bg-white rounded-4xl  overflow-y-scroll overflow-x-hidden"
                    @click.stop>
